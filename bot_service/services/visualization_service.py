...
def visualize_last_7_days(joined_csv_path: str, upto_ts: datetime | None = None) -> str | None:
    ...
    try:
        ...
        logger.error(f"Visualization error for {joined_csv_path}: {e}")
        return None


def visualize_dummy_mode(df: pd.DataFrame, strategy_name: str, upto_ts: datetime | None = None) -> str | None:
    """
    Create simple visualization for dummy mode.
    
    Top panel: Realized PnL and Unrealized PnL lines with rebalance points.
    Bottom panel: Unrealized PnL vs Threshold line (signals when unrealized >= threshold).
    
    Args:
        df: DataFrame with columns: realized_pnl, unrealized_pnl, rebalance_point
        strategy_name: Name of the strategy
        upto_ts: Upper bound timestamp for visualization
    ...
    try:
        ...
        # Use VIZ_WINDOW_DAYS from config
        import config
        ...
        # Top plot: PnL lines
        ax1 = fig.add_subplot(gs[0])
        if 'realized_pnl' in dff.columns:
            ax1.plot(dff.index, dff['realized_pnl'], label='Realized PnL', 
                    color='tab:blue', linewidth=2)
        if 'unrealized_pnl' in dff.columns:
            ax1.plot(dff.index, dff['unrealized_pnl'], label='Unrealized PnL', 
                    color='tab:green', linewidth=2)
        
        # Mark rebalance points
        if 'rebalance_point' in dff.columns:
            rp_idx = dff.index[dff['rebalance_point'] == True]
            if not rp_idx.empty and 'unrealized_pnl' in dff.columns:
                ax1.scatter(rp_idx, dff.loc[rp_idx, 'unrealized_pnl'], 
                           color='red', marker='o', s=100, label='Rebalance Signal', 
                           zorder=5, edgecolors='darkred', linewidths=2)
        
        ax1.set_title(f"Strategy: {strategy_name} — Dummy Mode (last {viz_days} days)", fontsize=14, fontweight='bold')
        ax1.set_ylabel('PnL ($)', fontsize=12)
        ax1.legend(loc='upper left', fontsize=10)
        ax1.grid(True, alpha=0.3)
        
        # Bottom plot: Unrealized PnL vs Threshold
        ax2 = fig.add_subplot(gs[1], sharex=ax1)
        if 'unrealized_pnl' in dff.columns:
            ax2.plot(dff.index, dff['unrealized_pnl'], label='Unrealized PnL', 
                    color='tab:green', linewidth=2)
            
            # Add threshold line
            threshold = config.DUMMY_PNL_THRESHOLD
            ax2.axhline(y=threshold, color='red', linestyle='--', linewidth=2, 
                       label=f'Threshold: ${threshold:,.0f}', alpha=0.8)
            
            # Mark rebalance points on bottom plot too
            if 'rebalance_point' in dff.columns:
                rp_idx = dff.index[dff['rebalance_point'] == True]
                if not rp_idx.empty:
                    ax2.scatter(rp_idx, dff.loc[rp_idx, 'unrealized_pnl'], 
                               color='red', marker='o', s=100, 
                               zorder=5, edgecolors='darkred', linewidths=2)
        
        ax2.set_ylabel('Unrealized PnL ($)', fontsize=12)
        ...
        # Save figure
        os.makedirs(config.VIZ_DIR, exist_ok=True)
        out_path = os.path.join(config.VIZ_DIR, f"{strategy_name}_dummy_{viz_days}d.png")
        fig.savefig(out_path, dpi=200, bbox_inches='tight')
        plt.close(fig)
        
        logger.debug(f"Created dummy mode visualization: {out_path}")
        return out_path
        
    except Exception as e:
        logger.error(f"Dummy visualization error for {strategy_name}: {e}")
        return None
...
def visualize_last_7_days_df(df: pd.DataFrame, strategy_name: str, upto_ts: datetime | None = None) -> str | None:
    ...
    try:
        ...
        logger.error(f"Visualization (DF) error for {strategy_name}: {e}")
        return None
